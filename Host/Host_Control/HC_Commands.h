    ...
    const uint8_t PING_STATE_All = 0x07; //PING STATE, ALL //IMPLEMENTED
     ...
     const uint8_t RECC_LAUNCH_PROCEDURE = 0x29; //print reccomended command sequence for launch
      ...
      const uint8_t PING_INJ_TEMP = 0x31; //ping the temeperature of the chamber by the injector
       ...
       const uint8_t  CHECK_GS_CONTINUITY = 0x92;
        const uint8_t  CHECK_MC_CONTINUITY = 0x91;
        const uint8_t  CHECK_MC_FLOAT = 0x80;
    ...
    const uint8_t LAUNCH_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x10, 0xDE, 0xFE};
     ...
     const uint8_t PING_TANK_PRESSURE_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x28, 0xD3, 0x4E}; //PING pressure
      ...
      // const uint8_t PING_VENT_POS_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x41, 0xC7, 0xEB};
       ...
       const uint8_t CHECK_MC_CONTINUITY_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x91, 0x4A, 0x4B};
       const uint8_t CHECK_GS_CONTINUITY_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x92, 0x4B, 0x4A};
       const uint8_t CHECK_MC_FLOAT_PACKET[7] = {0xAA, 0x14, 0x01, 0xAC, 0x80, 0x4C, 0x5E};
...
//MC STUFF
    ...
    const  uint8_t INJ_TEMP_PACKET[8] = {0xAA, 0x14, 0x02, 0xCC, 0x49, 0x07, 0xDF, 0xE7};  //non constant, the second data param = TEMP IN F/10, STARTS AS 2 ^
     ...
     const uint8_t ALL_DATA_PACKET[11] = {0xAA, 0x14, 0x04, 0xCC, 0x51, 0x02, 0x00, 0x07, 0x07, 0xD9, 0x9D};  //non constant, ALL DATA, IN TANK PRES, CHAMB PRES, INJ TEMP, NOZ TEMP ORDER
      ...
      const uint8_t MC_INJ_OVER_TEMP_WARNING[7] = {0xAA, 0x14, 0x01, 0xCC, 0x70, 0xE1, 0xC5};
      const uint8_t MC_OX_OVER_PRES_WARNING[7] = {0xAA, 0x14, 0x01, 0xCC, 0x71, 0xD1, 0xD5};
      ...
      const uint8_t MC_FLOAT_TRIPPED[7] = {0xAA, 0x14, 0x01, 0xCC, 0x80, 0xB8, 0xE8};
      const uint8_t MC_FLOAT_NOT_TRIPPED[7] = {0xAA, 0x14, 0x01, 0xCC, 0x79, 0xF8, 0xE8};
      const uint8_t MC_FLOAT_ENABLED[7] = {0xAA, 0x14, 0x01, 0xCC, 0x78, 0xA8, 0xE8};
      const uint8_t MC_FLOAT_DISABLED[7] = {0xAA, 0x14, 0x01, 0xCC, 0x77, 0xB8, 0xD8};
      const uint8_t MC_NOZ_OVER_TEMP_WARNING[7] = {0xAA, 0x14, 0x01, 0xCC, 0x65, 0xC1, 0xC5};
      const uint8_t MC_CHAMB_OVER_PRES_WARNING[7] = {0xAA, 0x14, 0x01, 0xCC, 0x64, 0xE1, 0xE5};
...
/*GS stuff*/
    ...
    const uint8_t GS_PAUSED[7] = {0xAA, 0x14, 0x01, 0xF5, 0x63, 0xA2, 0xD3};
    const uint8_t GS_DISCONNECTED[7] = {0xAA, 0x14, 0x01, 0xF5, 0x64, 0xA9, 0xD1};
      const uint8_t GS_CONTINUITY_GOOD[7] = {0xAA, 0x14, 0x01, 0xCC, 0x92, 0xB8, 0xB5};
      const uint8_t GS_NO_CONT_SOLENOID[7] = {0xAA, 0x14, 0x01, 0xF5, 0x86, 0xB4, 0xB4};
           const uint8_t GS_TANK_FULL_STATE[7] = {0xAA, 0x14, 0x01, 0xF5, 0x84, 0xB4, 0xB9};
      const uint8_t GS_NO_CONT_ACT[7] = {0xAA, 0x14, 0x01, 0xF5, 0x83, 0xB4, 0xB7};
      const uint8_t GS_DISCONNECTING[7] = {0xAA, 0x14, 0x01, 0xF5, 0x80, 0xA1, 0xD1};
